...
## En este código se esta usando el patron de diseño singlenton.
"""
Afirmo que se está usando singlenton porque se está restringiendo el uso de objetos y se está tomado como clase global 
a NMEA_State, instanciandose en class Waiting, class Header, class Body, class Checksum y class End
""" 
from __future__ import annotations
from typing import Optional, Iterable, Iterator, cast


class NMEA_State: ## clase base llamada NMEA_State que en las siguientes lineas de codigo va a heredear de ella. 
...
class Waiting(NMEA_State): ## clase waiting hereda de la clase NMEA_State
    def feed_byte(self, input: int) -> NMEA_State:
        if input == ord(b"$"):
...
class Header(NMEA_State): ## clase waiting hereda de la clase NMEA_State
    def __init__(self, message: "Message") -> None:
        self.message = message
        self.message.reset()
...
class Body(NMEA_State):## clase waiting hereda de la clase NMEA_State
    def feed_byte(self, input: int) -> NMEA_State:
        if input == ord(b"$"):
            return Header(self.message)
        if input == ord(b"*"):
...
class Checksum(NMEA_State):## clase waiting hereda de la clase NMEA_State
    def feed_byte(self, input: int) -> NMEA_State:
        if input == ord(b"$"):
            return Header(self.message)
        if input in {ord(b"\n"), ord(b"\r")}:
...
class End(NMEA_State):## clase waiting hereda de la clase NMEA_State
    def feed_byte(self, input: int) -> NMEA_State:
        if input == ord(b"$"):
            return Header(self.message)
...
class Message:
    ...
    @property  ##uso del decorador property
    def valid(self) -> bool:
        return (
            self.checksum_len == 2
    ...
    def __repr__(self) -> str:    ## evidencia del uso de un operator overloading, usando en este caso un _repr_ para fusionar diversas listas de la forma más óptima  posible
